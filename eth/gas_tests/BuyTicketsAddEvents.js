...
contract('Gas measurements', (accounts) => {
  ...
  it('Gas cost of ticket purchase as number of events increase', async () => {
    ...
    for(let i = 0; i < 1000; i++) {
      ...
      await eventC.create_event(id, title, [1000], [ticket_price.toString()], false, 0, true, true, deadline, {from:owner});
      let receipt = (await eventC.buy_tickets(id, 0, 1, {from: buyer, value: ticket_price})).receipt;
      costs.push(receipt.gasUsed);
    }
    content = costs.join('\n');
    fs.writeFileSync('buytickets_addevents.csv', content);
  });
});
